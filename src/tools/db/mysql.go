...
func Connect(dbName string) error {
	mysqlListConf := cfg.GetServerConfig().Mysql
	var find_db_index = -1
	for i := 0; i < len(mysqlListConf); i++ {
		if mysqlListConf[i].Name == dbName {
			find_db_index = i
			break
		}
	}
	...
	if find_db_index < 0 {
		return errors.New("Can't find db config")
	}
	mysqlConf := mysqlListConf[find_db_index]

	hostname := mysqlConf.IP
	port := mysqlConf.Port
...
func (m *Model) SetNull(param ...string) (num int, err error) {
	if m.db == nil {
